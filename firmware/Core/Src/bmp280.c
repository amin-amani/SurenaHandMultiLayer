...
static BMP280_CALLBACK_STRUCT_TYPE *bmp280;

void bmp280_register_callbacks(BMP280_CALLBACK_STRUCT_TYPE *bmp)
{
bmp280=bmp;
}
...
void BME280_CONFIG_SETUP(int index)
{
	...
	bmp280->spi_write(index,config,2);
...
void BME280_GET_COMP_VALS(int index)
{
	...
	bmp280->spi_read_write(index,config,1,comp,24);
	...
	bmp280->spi_read_write(index,config,1, &comp[24],1);
	dig_H1 = comp[24];

	config[0] = COMPHUMREST;

	bmp280->spi_read_write(index,config,1, &comp[25],7);
...
void BME280_GET_RAW_VALS(int index)
{
	...
	bmp280->spi_read_write(index, config, 1, tempread, 8);
...
bool write8(int8_t reg,uint8_t val,int index)
{
	...
	txBuffer[0]=reg& ~ 0x80;
	txBuffer[1]=val;
	bmp280->spi_write(index,txBuffer,2);
	return true;
...
{
	uint8_t rxBuffer[1];
	bmp280->spi_read_write(index,&reg, 1, rxBuffer, 1);
	return rxBuffer[0];
}
...
uint32_t read24(int8_t reg,int index)
{
	...
	uint8_t rxBuffer[3];
	txBuffer[0]=(reg|0x80);
	bmp280->spi_read_write(index, txBuffer, 1, rxBuffer, 3);
	result=rxBuffer[0];
	result<<=8;
	result|=rxBuffer[1];
	result<<=8;
...
int32_t read16(uint8_t reg,int index)
{
	...
	uint8_t rxBuffer[2];
	txBuffer[0]=reg;
	bmp280->spi_read_write(index, txBuffer, 1, rxBuffer, 2);
	result=rxBuffer[0];
	result<<=8;
	result|=rxBuffer[1];
	return result;
...
float bmp280_read_temperature(int index)
{
...
float bmp280_read_pressure(int index)
{
	int64_t var1, var2, p;
	  if (!_sensorID[index])
	    return 0;
	// Must be done first to get the t_fine variable set up
	bmp280_read_temperature(index);
